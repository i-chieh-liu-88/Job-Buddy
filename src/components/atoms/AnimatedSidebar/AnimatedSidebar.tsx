...
// eslint-disable-next-line react-refresh/only-export-components -- consumers need this official context hook.
...
export function AnimatedSidebarProvider({
  ...
  const motionReduce = useReducedMotion() ?? false;
  const reduce =
    motionReduce ||
    ...
    >
      ...
      <motion.div
        ...
        className={cn(
          ...
          "border-line bg-surface shadow-2xl will-change-transform",
...
export const AnimatedSidebar = forwardRef<HTMLElement, AnimatedSidebarProps>(
  function AnimatedSidebar(
    ...
    if (context.isMobile && desktopOnly) {
      return null;
    }
    ...
    return (
      ...
      >
        <motion.div
          ...
          className={cn(
            "sticky top-0 flex h-svh w-full flex-col overflow-hidden bg-surface",
            collapsible === "offcanvas" && "w-[var(--sidebar-width)]",
            variant === "sidebar" &&
              (side === "left" ? "border-line border-r" : "border-line border-l"),
            variant === "floating" &&
              "m-2 h-[calc(100svh-1rem)] rounded-2xl border border-line shadow-sm",
...
>(function AnimatedSidebarRail(
  { className, onClick, tabIndex = 0, title, type = "button", ...props },
  forwardedRef,
) {
  const context = useAnimatedSidebar();
  ...
  return (
    <button
      ...
      title={title ?? (props["aria-label"] ?? "Toggle sidebar")}
      tabIndex={tabIndex}
      onClick={(event) => {
        onClick?.(event);
        if (!event.defaultPrevented) context.toggleSidebar();
...
>(function AnimatedSidebarInset({ className, ...props }, forwardedRef) {
  return (
    <motion.main
      ...
      className={cn(
        "relative flex min-h-svh min-w-0 flex-1 flex-col bg-surface",
...
>(function AnimatedSidebarFooter({ className, ...props }, forwardedRef) {
  return (
    <div
      ...
      className={cn(
        "flex shrink-0 flex-col gap-2 border-line border-t p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]",
        className,
      )}
    />
  );
...
>(function AnimatedSidebarGroupLabel(
  ...
  return (
    <div
      ...
      className={cn(
        "mb-1 h-7 overflow-hidden px-2 text-[10px] font-medium uppercase tracking-[0.14em] text-muted transition-opacity",
...
>(function AnimatedSidebarMenu(
  ...
  return (
    <SharedLayoutBg
      ...
      as="ul"
      inset={0}
      pillClassName="rounded-xl bg-hover"
    ...
    <AnimatePresence initial={false} mode="popLayout">
      {open && !panel.collapsed ? (
        <motion.ul
          ...
          className={cn(
            "relative mt-1 ml-5 flex min-w-0 flex-col gap-0.5 border-line border-l pl-3",
            className,
          )}
        >
          {children}
...
export function AnimatedSidebarMenuSubButton({
  ...
  const interactiveClassName = cn(
    ...
    "text-muted transition-colors hover:bg-hover hover:text-ink",
    "focus-visible:bg-hover focus-visible:ring-2 focus-visible:ring-focus",
...
export function AnimatedSidebarMenuButton({
    ...
    <>
      {isActive ? (
        <motion.span
          ...
          className="absolute inset-0 rounded-xl bg-hover"
        />
      ) : null}
      {icon ? (
        <span
      ...
      {ariaExpanded !== undefined ? (
        <motion.span
          ...
          transition={context.reduce ? { duration: 0 } : SPRING_LAYOUT}
          className="relative z-10 grid size-4 shrink-0 place-items-center text-muted"
        >
  ...
  return href ? (
    <motion.a
      ...
      aria-disabled={disabled || undefined}
      aria-label={ariaLabel ?? (panel.collapsed ? textLabel : undefined)}
      title={title ?? (panel.collapsed ? textLabel : undefined)}
      tabIndex={disabled ? -1 : undefined}
      onClick={select}
      whileTap={context.reduce || disabled ? undefined : { scale: 0.98 }}
      transition={SPRING_PRESS}
